...
exports.myorders = (req, res) => {
  db.start.query(
    'select itemid,title,description,price,imglink,order_id from items join itemsbooked on items.itemid = itemsbooked.item_id where buyer_id = ?',
    [req.session.userId],
    (err, results, fields) => {
      if (err) {
        // throw err;
      ...
      db.start.query(
        ...
        (er, r) => {
          db.start.query(
            ...
            [item_id, row[0].seller_id, req.session.userId],
            (err, result) => {
              if (err) {
                console.log(err)
                return res.redirect('/')
...
exports.sell = (req, res) => {
  ...
  if (title && description && price && imglink) {
    db.start.query(
      'INSERT INTO ITEMS(seller_id,title,description,price,imglink,status) VALUES(?,?,?,?,?,true)',
      [req.session.userId, title, description, price, imglink],
      (err, results, fields) => {
        if (err) {
